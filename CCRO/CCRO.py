...
class PBFTAggregator:
    # Initiate the class
    def __init__(self, num_of_corrupt, type_of_byzantine):
        ...
        self.communication_node = self.commander_node_selector(self.nodes_list)
        self.type_of_byzantine = self.type_of_byzantine(type_of_byzantine, self.byzantine_nodes, self.commander_node, self.communication_node)
    
    # Generate node list
    def nodes_list(self, num_of_corrupt):
        nodes_list = []
        for i in range(0, 3*num_of_corrupt + 2):
            nodes_list.append(i)
        return nodes_list
    ...
    def communication_node_selector(self, nodes_list):
        temp_list = list(nodes_list)
        temp_list.remove(self.commander_node)
    ...
    def type_of_byzantine(self, type_of_byzantine, byzantine_nodes_list, commander_node, communication_node):
        # for i in commander_node:
        if commander_node in byzantine_nodes_list and type_of_byzantine == 0:
            self.byzantine_nodes.remove(commander_node)
    ...
    # Get commander_node from class instance
    def getCommunicationNode(self):
        return self.communication_node
